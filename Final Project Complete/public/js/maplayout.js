...
function MapLayout(listLayout){
    ...
    self.topOfList = listLayout.topOfList;
...
MapLayout.prototype.init = function(){
    ...
    self.schools = self.schoolSimilarityMatrix.map(function(d){
...
MapLayout.prototype.drawSchools = function(filteredSchools){

    var self = this;
    
    var schoolCircles = self.mapSchools.selectAll('circle')
	.data(filteredSchools);

    schoolCircles.exit().remove();
    ...
    //Creates the set of school locations.
    self.schools = filteredSchools.map(function(d){
	return self.projection([d[0].LONGITUDE, d[0].LATITUDE])});
    ...
    var schoolVoronoi = self.mapVoronoi.selectAll('path')
	.data(self.schoolDataVoronoi)

    schoolVoronoi.exit().remove();
